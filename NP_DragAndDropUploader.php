...
/**
  * NP_DragAndDropUploader ($Revision: 1.14 $)
  * by hsur ( http://blog.cles.jp/np_cles )
  * 
  * Based on upload.php (http://www.plupload.com)
  * 
  * $Id: NP_DragAndDropUploader.php,v 1.14 2012/07/28 17:23:42 hsur Exp $
*/

/*
  * Copyright (C) 2010-2012 CLES. All rights reserved.
  *
...
class NP_DragAndDropUploader extends NucleusPlugin {
  ...
  function event_BookmarkletExtraHead(&$data){
	...
	function getDescription() {
		return '[$Revision: 1.14 $]<br />' . NP_DRAGANDDROPUPLOADER_DESCRIPTION;
	}
	...
	function _jsonrpc_ok($fileName = '', $collection = '', $height = 0, $width = 0, $timestamp = false){
		if( $timestamp === false )
			echo('{"jsonrpc" : "2.0", "result" : {"file_name": "'.$fileName.'", "collection": '.$collection.', "height": '.$height.', "width": '.$width.'}, "id" : "id"}');
		else
			echo('{"jsonrpc" : "2.0", "result" : {"file_name": "'.$fileName.'", "collection": '.$collection.', "height": '.$height.', "width": '.$width.', "timestamp": '.$timestamp.'}, "id" : "id"}');
		exit;
	}
	...
	function _image_upload(){		
		...
		if( $CONF['MediaPrefix'] ){
			...
			if( $exif !== false && $exif["DateTimeOriginal"] && strtotime($exif["DateTimeOriginal"]) !== false ){
				$fileName = strftime("%Y%m%d-", strtotime($exif["DateTimeOriginal"])) . $fileName;
				$timestamp = strftime("%s000", strtotime($exif["DateTimeOriginal"]));
			} else {
		...
		$this->_jsonrpc_ok($fileName, $collection, $height, $width, $timestamp);
	}
}
